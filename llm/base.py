  ...
  "vacancies": [                         // список ролей; пустой если is_casting=false
    {
      ...
      "shooting_date": str|null          // ВАЖНОЕ ПОЛЕ. Дата + ВРЕМЯ
                                         // съёмки/смены КАК ТЕКСТ в том
                                         // виде как в посте.
                                         //
                                         // Примеры дат (заполняй ВСЕГДА
                                         // когда есть число и месяц):
                                         //   «3 мая», «13.05», «27-28 мая»,
                                         //   «14.05.2026», «13-14 июня».
                                         //
                                         // Если в посте указано и ВРЕМЯ —
                                         // включай его в это же поле через
                                         // запятую:
                                         //   «15 мая с 10:00 до 18:00»
                                         //     → "15 мая, 10:00-18:00"
                                         //   «3 мая, утром» → "3 мая, утром"
                                         //   «20 мая в 14:00» → "20 мая, 14:00"
                                         //   «27.05, ночная смена 22-6»
                                         //     → "27.05, 22-6"
                                         //   «13.05 (репетиция)» → "13.05"
                                         //     (помета в скобках не нужна)
                                         //
                                         // Маркеры в тексте: «Дата:»,
                                         // «На <число>», «Съёмки <число>»,
                                         // «Смены <число>», «<число> мая»,
                                         // «<dd.mm>», «<HH:MM>», «с N до M».
                                         // Если в посте несколько дат —
                                         // объедини через запятую:
                                         // «22-23, 29-30 мая». Если есть
                                         // и дата объявления и дата
                                         // смены — бери ДАТУ СМЕНЫ.
                                         // null ТОЛЬКО если ни даты ни
                                         // времени вообще нет.
                                         // НЕ переводи в дату-объект, не
                                         // ISO, не нормализуй формат.
    }
  ]
}
...
"role_label": "Частник", "shooting_date": "2.06"}]}
...
_TIME_PATTERNS = (
    # «с 10:00 до 18:00», «с 9 до 22», «10:00-18:00», «10-18»
...
def _scan_shooting_date(text: str) -> str | None:
    ...
    Собирает ВСЕ найденные даты и времена, дедуплицирует, объединяет
    запятой. Возвращает None если ничего не нашёл."""
    import re
    if not text:
        return None
    seen: set[str] = set()
    out: list[str] = []
    for pat in _DATE_PATTERNS + _TIME_PATTERNS:
